...
MODEL_PATH='pretrained_models/vit_l_hybrid_pt_800e_k700_ft.pth'
OUTPUT_DIR='checkpoints/recog_dash_4/'
...
OUTPUT_DIR='checkpoints/recog_right_4/'
OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 \
    --master_port 12320 --nnodes=1  --node_rank=0 --master_addr=localhost \
    run_class_finetuning.py \
    --model vit_large_patch16_224 \
...
OUTPUT_DIR='checkpoints/recog_rear_4/'
OMP_NUM_THREADS=1 CUDA_VISIBLE_DEVICES=0,1,2,3 python -m torch.distributed.launch --nproc_per_node=4 \
    --master_port 12320 --nnodes=1  --node_rank=0 --master_addr=localhost \
    run_class_finetuning.py \
    --model vit_large_patch16_224 \
