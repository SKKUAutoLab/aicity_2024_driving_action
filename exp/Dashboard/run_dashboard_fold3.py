...
_FILENAME_TO_ID = {'Dashboard_user_id_47158_NoAudio_5': 1, 'Dashboard_user_id_47158_NoAudio_7': 2, 'Dashboard_user_id_47457_NoAudio_5': 3, 'Dashboard_user_id_47457_NoAudio_7': 4, 'Dashboard_user_id_52046_NoAudio_11': 5,
...
def main():
    ...
    with open("pred_dash_fold3.txt", "w") as fp:
        for (vid, label, start, end) in loc_segments:
            fp.writelines("{} {} {} {}\n".format(int(vid), label, start, end))
    df = pd.read_csv('pred_dash_fold3.txt', delimiter=' ', names=["ID", "Label (Primary)", "Start Time", "End Time"])
    df = df.sort_values(by=['ID', 'Start Time'])
    df.to_csv('pred_dash_fold3.txt', index=False, header=False, sep=' ')
