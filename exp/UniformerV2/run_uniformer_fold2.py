...
def main():
    clip_classification = []
    pickle_dir_videomae = "pickles_uniformer_fold2/A2"
    ...
    with open("pred_uniformer_fold2.txt", "w") as fp:
        for (vid, label, start, end) in loc_segments:
            fp.writelines("{} {} {} {}\n".format(int(vid), label, start, end))
    df = pd.read_csv('pred_uniformer_fold2.txt', delimiter=' ', names=["ID", "Label (Primary)", "Start Time", "End Time"])
    df = df.sort_values(by=['ID', 'Start Time'])
    df.to_csv('pred_uniformer_fold2.txt', index=False, header=False, sep=' ')
