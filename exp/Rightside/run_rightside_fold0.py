...
_FILENAME_TO_ID = {'Right_side_window_user_id_20090_NoAudio_5': 1, 'Right_side_window_user_id_20090_NoAudio_7': 2,
...
def main():
    ...
    with open("pred_right_fold0.txt", "w") as fp:
        for (vid, label, start, end) in loc_segments:
            fp.writelines("{} {} {} {}\n".format(int(vid), label, start, end))
    df = pd.read_csv('pred_right_fold0.txt', delimiter=' ', names=["ID", "Label (Primary)", "Start Time", "End Time"])
    df = df.sort_values(by=['ID', 'Start Time'])
    df.to_csv('pred_right_fold0.txt', index=False, header=False, sep=' ')
