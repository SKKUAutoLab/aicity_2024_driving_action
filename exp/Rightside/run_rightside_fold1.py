...
_FILENAME_TO_ID = {'Right_side_window_user_id_20931_NoAudio_5': 1, 'Right_side_window_user_id_20931_NoAudio_7': 2, 'Right_side_window_user_id_41850_NoAudio_5': 3, 'Right_side_window_user_id_41850_NoAudio_7': 4,
...
def main():
    ...
    with open("pred_right_fold1.txt", "w") as fp:
        for (vid, label, start, end) in loc_segments:
            fp.writelines("{} {} {} {}\n".format(int(vid), label, start, end))
    df = pd.read_csv('pred_right_fold1.txt', delimiter=' ', names=["ID", "Label (Primary)", "Start Time", "End Time"])
    df = df.sort_values(by=['ID', 'Start Time'])
    df.to_csv('pred_right_fold1.txt', index=False, header=False, sep=' ')
