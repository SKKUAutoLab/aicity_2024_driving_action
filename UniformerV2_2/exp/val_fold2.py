...
class VideoInferDataset(Dataset):
    def __init__(self, data_root, video_csv_path, clip_len=8, frame_sample_rate=16, clip_stride=30, short_side_size=336,
                 new_height=336, new_width=336, keep_aspect_ratio=True, crop=False, view="dashboard"):
...
def main(cfg, data_path, csv_file, sampling_rate, clip_stride, crop, view, fold, checkpoint_file_path):
    ...
    test_loader = torch.utils.data.DataLoader(dataset, batch_size=8, num_workers=4)
    row_data = []
    # load model
    ...
    output_dir = 'pickles_uniformer_fold2/A2'
    if not os.path.exists(output_dir):
        os.makedirs('pickles_uniformer_fold2/A2')
...
if __name__ == '__main__':
    ...
    path_saved_ckpt = natsorted(glob.glob('full_ckpt/*.pyth'))
    ...
    for i in range(len(checkpoint_dashboard_list)):
        ...
        main(cfg, 'data/Val_Fold_2/', 'video_ids.csv', cfg.DATA.SAMPLING_RATE, 30, True, 'dash', i, checkpoint_dashboard_list[i])
    for i in range(len(checkpoint_rearview_list)):
        print('Load checkpoint:', checkpoint_rearview_list[i])
        main(cfg, 'data/Val_Fold_2/', 'video_ids.csv', cfg.DATA.SAMPLING_RATE, 30, True, 'rear', i, checkpoint_rearview_list[i])
    for i in range(len(checkpoint_right_list)):
        print('Load checkpoint:', checkpoint_right_list[i])
        main(cfg, 'data/Val_Fold_2/', 'video_ids.csv', cfg.DATA.SAMPLING_RATE, 30, True, 'right', i, checkpoint_right_list[i])
