...
def main(cfg, data_path, csv_file, sampling_rate, clip_stride, crop, view, fold, checkpoint_file_path):
    ...
    output_dir = 'pickles_uniformer_fold1/A2'
    if not os.path.exists(output_dir):
        os.makedirs('pickles_uniformer_fold1/A2')
...
if __name__ == '__main__':
    ...
    for i in range(len(checkpoint_dashboard_list)):
        ...
        main(cfg, 'data/Val_Fold_1/', 'video_ids.csv', cfg.DATA.SAMPLING_RATE, 30, True, 'dash', i, checkpoint_dashboard_list[i])
    for i in range(len(checkpoint_rearview_list)):
        print('Load checkpoint:', checkpoint_rearview_list[i])
        main(cfg, 'data/Val_Fold_1/', 'video_ids.csv', cfg.DATA.SAMPLING_RATE, 30, True, 'rear', i, checkpoint_rearview_list[i])
    for i in range(len(checkpoint_right_list)):
        print('Load checkpoint:', checkpoint_right_list[i])
        main(cfg, 'data/Val_Fold_1/', 'video_ids.csv', cfg.DATA.SAMPLING_RATE, 30, True, 'right', i, checkpoint_right_list[i])
