...
with open("A2_dash_vmae_16x4_crop_fold0.pkl", "rb") as f:
    prob_data = pickle.load(f)
video_key = 'Dashboard_user_id_12670_NoAudio_7'  
prob_list = prob_data[video_key]
prob_array = np.array([np.array(frame_probs, dtype=np.float32) for frame_probs in prob_list])
video_path = "Dashboard_user_id_12670_NoAudio_7.MP4"
cap = cv2.VideoCapture(video_path)
fps = int(cap.get(cv2.CAP_PROP_FPS))
frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
output_path = "output_Dashboard_user_id_12670_NoAudio_7.mp4"
